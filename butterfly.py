...
#尝试我自己写的plot_vedio
from plot_video_GJM import plot_video_2D
...
n_elem = 3 # Change based on requirements, but be careful
n_elem += n_elem % 2
...
positions = np.empty((MaxDimension.value(), n_elem + 1)) #相当于别的地方函数的position_collection
dl = total_length / n_elem

# First half of positions stem from slope angle_of_inclination 前半段位置源于斜坡倾角 angle_of_inclination = np.deg2rad(45.0)
...
butterfly_sim.collect_diagnostics(butterfly_rod).using(
    VelocityCallBack, step_skip=10000, callback_params=recorded_history
)
...
if PLOT_FIGURE:
    ...
    ax.plot(first_position[2, ...], first_position[0, ...], "r--", lw=2.0)#红色虚线是初始位置
    ...
    ax.plot(last_position[2, ...], last_position[0, ...], "k--", lw=2.0)#最后位置是黑色虚线
    # don't block
    fig.show()
